...
def overwrite_config(config, prev_config):
    ...
    for key in vars(config).keys():
        if not key in vars(prev_config).keys():
            ...
            config_value = vars(config)[key]
            #if '--%s' % key in sys.argv:
            #if key in ['init_epoch']:
            if vars(config)[key] != 'NA':
...
def continue_main(config, main):    # If the model exists, load model and configuration to continue the training.
...
if __name__ == '__main__':
    ...
    config = cfg.continue_config(batch_size=64, load_fn='model_064_16_1.53-4.62_1.63-5.09_pth', init_epoch=17, n_epochs=20, gpu_id=0)
